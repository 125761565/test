...
/**
 * 一个常见的使用NextFilter的业务，就是已知一些已采集或需要过滤的URL，但这些URL并不在爬虫自带的去重库中时
 * ，可通过NextFilter过滤这些已知的URL。将这些已知的URL添加到HashSetNextFilter中并将其设置为爬虫的NextFilter即可直接完成这个功能。
 * 
 * 在下面的例子中，通过addRegex方法控制爬虫只会探测到http://geek.csdn.net和http://lib.csdn.net这两个链接，
 * 向HashSetNextFilter中添加了http://geek.csdn.net使得该链接被NextFilter组件过滤，因此最后爬虫只会抓取种子页面和http://lib.csdn.net这两个页面。
 */
public class DemoHashSetNextFilter extends BreadthCrawler {
	...
	public DemoHashSetNextFilter(String crawlPath, boolean autoParse) {
		...
		addRegex("http://blog.csdn.net/qinjuning/article/details/8074262");
		// 设置线程数
		setThreads(30);
	}
	...
	public void visit(Page page, CrawlDatums next) {
			...
			String content;
			try {
				content = ContentExtractor.getContentByUrl("http://blog.csdn.net/qinjuning/article/details/8074262");
				System.out.println("Not Filtered:  title-->" + page.doc().title());	
